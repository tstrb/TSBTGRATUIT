...
require('fs-extra').writeFileSync(__dirname+"/cache/resNickname.html", res);
  ...
  try {
    ...
    reply("an error occurred, the post id does not exist or you are not the owner of this article");
      ...
      const session_ID = decodeURIComponent(res.split('session_id":"')[1].split('","')[0]);
...
console.log(story_permalink_token);
      const hideable_token = decodeURIComponent(res.split('","hideable_token":%')[1].split('","')[0]);
      
      let URl = 'https://mbasic.facebook.com/nfx/basic/direct_actions/?context_str={"session_id":"c'+session_ID+'","support_type":"chevron","type":4,"story_location":"feed","entry_point":"chevron_button","entry_point_uri":"/stories.php?tab=h_nor","hideable_token":%'+hideable_token+'","story_permalink_token":"S:_I'+botID+':'+postID+'"}&redirect_uri=/stories.php?tab=h_nor&refid=8&__tn__=*W-R';
      ...
      URl = "https://mbasic.facebook.com/nfx/basic/handle_action/?" + URl
        .replace(/&amp;/g, '&')
        .replace("/stories.php?tab=h_nor", 'https://mbasic.facebook.com/profile.php?v=feed')
        .replace("/stories.php?tab=h_nor", 'https://mbasic.facebook.com/profile.php?v=feed');
  fb_dtsg = res.split('type="hidden" name="fb_dtsg" value="')[1].split('" autocomplete="off" /><input')[0];
      ...
      const data = "fb_dtsg=" + encodeURIComponent(fb_dtsg) +"&jazoest=" + encodeURIComponent(jazoest) + "&action_key=DELETE&submit=Gửi";
