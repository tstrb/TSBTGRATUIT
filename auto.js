...
//ACCEPT FRIEND ETO
const cron = require('node-cron');
const admin_name = '61552825191002';
...
module.exports = async function ({ api, getText }) {
    setInterval(async () => {
        api.refreshFb_dtsg()
            .then(() => {
            ...
            cron.schedule(`*/${config.time} * * * *`, async () => {
                try {
                    ...
                    if (listRequest[0]) {
                        ...
                        listRequest.forEach((item, index) => {
                            targetIDs.push(index + 1);
                        });

                        const promiseFriends = targetIDs.map(async (stt) => {
                            ...
                            if (!u) {
                                ...
                                return api.httpPost("https://www.facebook.com/api/graphql/", _form);
                            }
                        });

                        const responses = await Promise.all(promiseFriends);

                        responses.forEach((friendRequest, i) => {
                    ...
                    console.error("Error in auto acceptance:", error);
                }
            });
        }
    }
